...
public:
    // add session
//    virtual bool insertSessionInfo(const st::Entity::ImSessionInfo &imSessionInfo) = 0;
//    virtual bool bulkInsertSessionInfo(const std::vector<st::Entity::ImSessionInfo> &sessionList) = 0;
    // delete session
    virtual bool bulkDeleteSessions(const std::vector<std::string> &peerIds) = 0;
    // recent session contains user and group
    virtual void getRecentSession(std::vector<st::StShareSession> &sessions) = 0;
    // recent session contains user only
    virtual void geContactsSession(std::vector<st::StShareSession> &sessions) = 0;
    // make and get session
//    virtual std::shared_ptr<std::vector<std::shared_ptr<st::Entity::ImSessionInfo> > > QueryImSessionInfos() = 0;
    // get session
    virtual std::shared_ptr<std::vector<std::shared_ptr<st::entity::ImSessionInfo> > > reloadSession() = 0;

    // message
public:
    // add message
    virtual bool insertMessageInfo(const st::entity::ImMessageInfo &imMessageInfo) = 0;
    virtual bool bulkInsertMessageInfo(const std::vector<st::entity::ImMessageInfo> &msgList) = 0;
    // delete message by message id
    virtual bool deleteMessageByMessageId(const std::string &meesageId) = 0;
    // get max timestamp by chat type
    virtual long long getMaxTimeStampByChatType(st::Enum::ChatType chatType) = 0;
    // get message by id
    virtual bool getUserMessage(const long long &time, const std::string &userName,const std::string &realJid,
                                std::vector<st::entity::ImMessageInfo> &msgList) = 0;
    ...
    virtual void getSearchMessage(const long long &time, const std::string &userId, const std::string &realJid,
                                  const std::string& searchKey, std::vector<st::entity::ImMessageInfo> &msgList) = 0;
    ...
    virtual bool getMessageByMessageId(const std::string &messageId, st::entity::ImMessageInfo &imMessageInfo) = 0;
...
public:
    // add
    virtual bool insertUserInfo(const st::entity::ImUserInfo &userInfo) = 0;
    virtual bool bulkInsertUserInfo(const std::vector<st::entity::ImUserInfo> &userInfos) = 0;
    virtual bool setUserCardInfo(const std::vector<st::StUserCard> &userInfos) = 0;
    // delete
    ...
    // get user info by id
    virtual std::shared_ptr<st::entity::ImUserInfo> getUserInfoByXmppId(const std::string &xmppids) = 0;
    // update user mood
    ...
    //
    virtual bool insertOrUpdateUserSuppl(std::shared_ptr<st::entity::ImUserSupplement> imUserSup) = 0;
    //
    virtual bool getStructure(std::vector<std::shared_ptr<st::entity::ImUserInfo>> &structure) = 0;

    // group
public:
    // add
    virtual bool insertGroupInfo(const st::entity::ImGroupInfo &userInfo) = 0;
    virtual bool bulkInsertGroupInfo(const std::vector<st::entity::ImGroupInfo> &userInfos) = 0;
    ...
    virtual std::shared_ptr<st::entity::ImGroupInfo> getGroupInfoByXmppId(const std::string &xmppids) = 0;
    // update group card
    virtual bool updateGroupCard(const std::vector<st::entity::ImGroupInfo> &groups) = 0;
    //
    virtual bool
    getGroupMemberInfo(std::map<std::string, st::StUserCard> &userInfos) = 0;
    // get group topic
    ...
    // all group
    virtual bool getAllGroup(std::vector<st::entity::ImGroupInfo> &groups) = 0;

    virtual bool getGroupCardById(std::shared_ptr<st::entity::ImGroupInfo> &group) = 0;
...
public:
    ...
    // get group card
    virtual bool getGroupMemberById(const std::string &groupId, std::map<std::string, st::StUserCard> &member,
                                    std::map<std::string, QUInt8> &userRole) = 0;
    virtual bool
    getGroupMemberInfo(const std::vector<std::string> &members, std::vector<st::StUserCard> &userInfos) = 0;
    //
...
public:
    ...
    virtual bool bulkInsertConfig(const std::vector<st::entity::ImConfig> &config) = 0;
    ...
    virtual bool getAllConfig(std::vector<st::entity::ImConfig> &configs) = 0;

    // friend
//public:
//    virtual bool bulkInsertFriends(const std::vector<st::Entity::IMFriendList> &friends) = 0;
//    virtual bool insertFriend(st::Entity::IMFriendList imfriend) = 0;
//    virtual bool deleteAllFriends() = 0;
//    virtual bool deleteFriendByXmppId(const std::string &xmppId) = 0;
//    virtual bool getAllFriends(std::vector<st::Entity::IMFriendList> &friends) = 0;
...
public:
    ...
    virtual void getQuickGroups(std::vector<st::entity::ImQRgroup>& groups) = 0;
    virtual void getQuickContentByGroup(std::vector<st::entity::IMQRContent>& contents,int id) = 0;

public:
    virtual void insertMedalList(const std::vector<st::entity::ImMedalList>& medals) = 0;
    virtual void insertMedals(const std::vector<st::entity::ImUserStatusMedal>& medals) = 0;
    virtual void getUserMedal(const std::string& xmppId, std::set<st::StUserMedal>& stMedal) = 0;
    virtual void getMedalList(std::vector<st::entity::ImMedalList>& medals) = 0;
    virtual void getMedalUsers(int medalId, std::vector<st::StMedalUser>& metalUsers) = 0;
