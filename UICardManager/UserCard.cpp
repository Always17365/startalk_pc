...
bool user_card::showUserCard(std::shared_ptr<st::entity::ImUserSupplement> imuserSup,
                             std::shared_ptr<st::entity::ImUserInfo> info)
{
    ...
    bool ret = false;
    st::entity::JID jid(imuserSup->XmppId);
    //
    _isSelf = jid.username() == DC.getSelfUserId();
...
void user_card::showMedal(const std::set<st::StUserMedal> &user_medal)
{
...
bool user_card::eventFilter(QObject *o, QEvent *e)
{
    if (o == _pDepartmentEdit) {
        if (e->type() == QEvent::MouseButtonPress) {
            ...
            if (DC.getShowStaff() && evt && evt->button() == Qt::LeftButton) {
...
void user_card::closeEvent(QCloseEvent *e)
{
    ...
    bool isSelf = _strUserId == DC.getSelfXmppId();
...
void user_card::modifyUserMedalStatus(int id, bool isWear)
{
    ...
    if (isWear && wear_count >= 3) {
        QtMessageBox::warning(this, QObject::tr("警告"), tr("最多佩戴三枚勋章, 请卸下其他勋章之后再操作"));
        return;
    }
    ...
    if (ret) {
        ...
        auto itFind = std::find_if(_user_medal.begin(), _user_medal.end(), [id](const st::StUserMedal & stUserMedal) {
        ...
        _user_medal.insert(st::StUserMedal(id, isWear ? 0x03 : 0x01));
        _pUserMedalWnd->init_medals(_user_medal);
    } else {
        QtMessageBox::warning(this, QObject::tr("警告"), tr("操作失败!"));
    }
}
...
void user_card::onShowMedalDetail(int id)
{
    if (nullptr == _pMedalDetailWnd) {
        ...
        qRegisterMetaType<std::vector<st::StMedalUser>>("std::vector<st::StMedalUser>");
    ...
    //
    auto itFind = std::find_if(_user_medal.begin(), _user_medal.end(), [id](const st::StUserMedal & it) {
    ...
    QtConcurrent::run([this, id]() {
        std::vector<st::StMedalUser> users;
        UserCardMsgManager::getMedalUser(id, users);
        _pMedalDetailWnd->setMedalUsers(id, users);
    });
}

void user_card::onShowUserList(const std::vector<st::StMedalUser> &useList)
{
