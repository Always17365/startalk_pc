...
database::database(const std::string &filename, const int flags,
                   const int timeoutMs,
                   const std::string &zVfs) : _filename(filename)
{
    const int ret = sqlite3_open_v2(filename.c_str(), &_sqlite, flags,
                                    zVfs.empty() ? nullptr : zVfs.c_str());

    if (SQLITE_OK != ret) {
        const st::sqlite::exception exception(_sqlite, ret);
    ...
    if (false == mbDone) {
        ...
        if (SQLITE_ROW == ret) { // one row is ready : call getColumn(N) to access it
            mbHasRow = true;
                   ...
                   ret) { // no (more) row ready : the query has finished executing
            mbHasRow = false;
            mbDone = true;
        } else {
            mbHasRow = false;
...
{
    const char *pText = reinterpret_cast<const char *>(sqlite3_column_text(mStmtPtr,
                                                       mIndex));
...
std::string column::getString() const
{
    ...
    const char *data = static_cast<const char *>(sqlite3_column_blob(mStmtPtr,
                                                 mIndex));
    // SQLite docs: "The safest policy is to invoke… sqlite3_column_blob() followed by sqlite3_column_bytes()"
    // Note: std::string is ok to pass nullptr as first arg, if length is 0
    return std::string(data,
                       static_cast<unsigned long>(sqlite3_column_bytes(mStmtPtr, mIndex)));
}
    ...
    mpRefCount(nullptr)
{
    const int ret = sqlite3_prepare_v2(apSQLite, aQuery.c_str(),
                                       static_cast<int>(aQuery.size()), &mpStmt,
                                       nullptr);
