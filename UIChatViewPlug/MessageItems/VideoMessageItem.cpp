...
protected:
    void paintEvent(QPaintEvent *event) override
    {
        ...
        if (!_imgPath.isEmpty()) {
            QPixmap pix = st::qimage::loadImage(_imgPath, false);
        ...
        qreal radius = 15;
        QRectF prect(width() / 2.0 - radius, height() / 2.0 - radius, 2 * radius,
                     2 * radius);
...
VideoMessageItem::VideoMessageItem(const StNetMessageResult &msgInfo,
                                   QWidget *parent)
...
void VideoMessageItem::initContentLayout()
{
    ...
    if (jsonDocument.isNull()) {
        ...
        _contentFrm->setFixedSize(10 + 10 + _contentLab->width(),
                                  10 + 10 + _contentLab->height());
        ...
        if (_videoImgLab == nullptr) {
            ...
            connect(this, &VideoMessageItem::sgDownloadedIcon, this,
                    &VideoMessageItem::setIcon, Qt::QueuedConnection);
            QString placeHolder = ":/chatview/image1/defaultShareIcon.png";

            if (thumbUrl.isEmpty()) {
                maskFrame->setImage(placeHolder);
            } else {
                std::string imgPath = st::GetImagePathByUrl(thumbUrl.toStdString());
...
void VideoMessageItem::playVideo()
{
    ...
    if (!jsonDocument.isNull()) {
        ...
        if (jsonObject.contains("newVideo") && jsonObject.value("newVideo").toBool()) {
            ...
            //
            if (videoUrl.isEmpty()) {
                QtMessageBox::warning(g_pMainPanel, QObject::tr("警告"),
                                      tr("无效的视频文件!"));
            } else {
                //
                localVideo = st::getVideoPathByUrl(videoUrl.toStdString()).data();
                //
...
void VideoMessageItem::setProcess(double speed, double dtotal, double dnow,
