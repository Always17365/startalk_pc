...
#endif
{
    ...
    connect(this, &MainWindow::sgRestartWithMessage, this,
            &MainWindow::restartWithMessage, Qt::QueuedConnection);
    connect(this, &MainWindow::sgCheckUpdate, this, &MainWindow::onCheckUpdater,
            Qt::QueuedConnection);
    //
...
void MainWindow::connectPlugs()
{
    ...
    connect(_chatViewPanel, SIGNAL(sgSwitchCurFun(int)),
            this, SLOT(onCurFunChanged(int)));
    connect(_titleBar, SIGNAL(sgJumpToNewMessage()), _navigationPanel,
            SLOT(jumpToNewMessage()));
    connect(this, SIGNAL(appDeactivated()), _titleBar, SLOT(onAppDeactivated()));
    connect(this, SIGNAL(appDeactivated()), _chatViewPanel,
            SLOT(onAppDeactivated()));
    connect(this, SIGNAL(appDeactivated()), _navigationPanel,
            SLOT(onAppDeactivated()));
    connect(this, SIGNAL(appDeactivated()), _pSysTrayIcon,
            SLOT(onAppDeactivated()));
    ...
    connect(_navigationPanel, SIGNAL(updateTotalUnreadCount(int)), _titleBar,
            SLOT(updateUnreadCount(int)));
    connect(_titleBar, SIGNAL(feedbackLog(const QString &)), _chatViewPanel,
            SLOT(packAndSendLog(const QString &)));
    ...
    connect(_titleBar, SIGNAL(systemQuitSignal()), this, SLOT(systemQuit()),
            Qt::QueuedConnection);
    connect(this, SIGNAL(systemQuitSignal()), this, SLOT(systemQuit()),
            Qt::QueuedConnection);
    ...
    connect(_titleBar, SIGNAL(sgSetAutoLogin(bool)), this,
            SLOT(setAutoLogin(bool)));
    connect(_titleBar, SIGNAL(sgUpdateHotKey()), this, SLOT(onUpdateHotKey()));
    connect(_titleBar, SIGNAL(sgSaveSysConfig()), GlobalManager::instance(),
            SLOT(saveSysConfig()));
    connect(_titleBar, SIGNAL(sgSaveSysConfig()), this, SLOT(onSaveSysConfig()));
    connect(_titleBar, SIGNAL(msgSoundChanged()), _chatViewPanel,
    ...
    connect(GlobalManager::instance(), SIGNAL(sgMousePressGlobalPos(QPoint)),
            _titleBar,
    ...
    connect(_pView, SIGNAL(sgShowUpdateClientLabel(bool)), _titleBar,
            SLOT(onShowUpdateLabel(bool)));
    connect(_titleBar, SIGNAL(sgDoUpdateClient()), this,
            SLOT(onShowCheckUpdateWnd()));
    // 自动回复
    connect(_titleBar, SIGNAL(sgAutoReply(bool)), _chatViewPanel,
            SLOT(setAutoReplyFlag(bool)));
    // 操作日志
    connect(_titleBar, SIGNAL(sgOperator(const QString &)), this,
            SLOT(addOperatorLog(const QString &)));
    connect(_chatViewPanel, SIGNAL(sgOperator(const QString &)), this,
            SLOT(addOperatorLog(const QString &)));
    connect(_navigationPanel, SIGNAL(sgOperator(const QString &)), this,
            SLOT(addOperatorLog(const QString &)));
    connect(_pCardManager, SIGNAL(sgOperator(const QString &)), this,
            SLOT(addOperatorLog(const QString &)));
    ...
    connect(_pOfflineTimer, &QTimer::timeout, [this]()
    {
        //        if(_pMessageManager)
        {
            ...
            QTalkMsgManager::sendOnlineState(_login_t, _logout_t,
                                             _ipv4Address.toStdString());
        }
    });
    connect(_pLogTimer, &QTimer::timeout, this, &MainWindow::onHourTimer);
    //
#ifdef TSCREEN
    connect(_pOAManager, SIGNAL(sgShowThrowingScreenWnd()), this,
            SLOT(startTScreen()));
    connect(_titleBar, SIGNAL(sgShowThrowingScreenWnd()), this,
            SLOT(startTScreen()));
#endif
}
...
#ifdef _MACOS
    connect(_pFeedBackLog, &QAction::triggered, _pSysTrayIcon,
            &SystemTray::onSendLog);
#endif
}
...
void MainWindow::openMainWindow()
{
    if (_initUi)
    {
        ...
        connect(_pLocalServer, &LocalServer::sgWakeupWindow, this,
        ...
        connect(this, &MainWindow::sgResetOperator,
                ...
                Qt::QueuedConnection);
        connect(_noOperatorThread, SIGNAL(sgUserLeave(bool)), _chatViewPanel,
                SLOT(setAutoReplyFlag(bool)));
        connect(_noOperatorThread, SIGNAL(sgUserLeave(bool)), this,
                SLOT(setUserStatus(bool)));
        connect(_titleBar, SIGNAL(sgAutoReply(bool)), _noOperatorThread,
                SLOT(setAutoReplyFlag(bool)));
        //
#ifdef _MACOS
