...
public:
    ...
    inline void bindNoCopy(const std::string &aName, const void *apValue,
                           const int aSize)
    {
    ...
    inline void checkIndex(const int aIndex) const
    {
        if ((aIndex < 0) || (aIndex >= mColumnCount)) {
            throw st::sqlite::exception("Column index out of range.");
        }
    }
...
private:
    ...
    private:
        ...
        unsigned int
        *mpRefCount;  //!< Pointer to the heap allocated reference counter of the sqlite3_stmt
    ...
    mutable std::map<std::string, int>
    mColumnNames;   //!< Map of columns index by name (mutable so getColumnIndex can be const)
