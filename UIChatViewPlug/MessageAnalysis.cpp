...
void analysisMessage(StNetMessageResult &info)
{
    ...
    case st::entity::MessageTypeRevoke: {
        ...
        QString content = QObject::tr("%1撤回了一条消息").arg(
                              info.from == DC.getSelfXmppId().data() ?
    ...
    case st::entity::MessageTypeFile: {
        st::analysisFileMessage(info.body, info.extend_info, info);
        break;
    }

    case st::entity::MessageTypeCommonTrdInfo:
    case st::entity::MessageTypeCommonTrdInfoV2: {
        st::analysisCommonTrdMessage(info.body, info.extend_info, info);
        break;
    }

    case st::entity::MessageTypeSourceCode: {
        st::analysisCodeMessage(info.body, info.extend_info, info);
        break;
    }

    case st::entity::MessageTypeSmallVideo: {
        st::analysisVideoMessage(info.body, info.extend_info, info);
        break;
    }
    ...
    case st::entity::MessageTypeShock: {
        ...
        if (DC.getSelfXmppId() == msgFrom.toStdString()) {
    ...
    case st::entity::MessageTypeRobotAnswer: {
        bool isSendMessage = info.direction == st::entity::MessageDirectionSent
                             && info.state == 0;
...
void analysisTextMessage(const QString &content,
                         std::vector<StTextMessage> &messages, const std::map<QString, QString> &atUsers)
{
    ...
    while ((pos = regExp.indexIn(content, pos)) != -1) {
        // 前面的文本
        QString text = content.mid(prePos,
                                   pos - prePos); // 取匹配开始位置到匹配到的位置文本
        ...
        if ("url" == type) {
            ...
            if (pkgid.isEmpty() || shortCut.isEmpty()) {
                ...
                QString localEmo = EmoticonMainWgt::instance()->getEmoticonLocalFilePath(pkgid,
                                   shortCut);
                ...
                if (localEmo.isEmpty() || !info.exists() || info.isDir()) {
                    ...
                    st::Image::scaImageSize(emoMsg.imageWidth, emoMsg.imageHeight);
                }
            ...
            analysisImageMessage(item, imageMsg.imageLink, imageMsg.imageWidth,
                                 imageMsg.imageHeight);

            QString localImg = st::GetImagePathByUrl(
                                   imageMsg.imageLink.toStdString()).data();
...
void  analysisSendTextMessage(const QString &extendInfo,
                              std::vector<StTextMessage> &messages)
{
        ...
        for (auto &&i : array) {
            ...
            case Type_Image: {
                ...
                if (!info.exists() || info.isDir()) {
                    ...
                    st::Image::scaImageSize(imageMsg.imageWidth, imageMsg.imageHeight);
                }
...
void analysisLinkMessage(const QString &text,
                         std::vector<StTextMessage> &messages, const std::map<QString, QString> &atUsers)
{
    if (!text.isEmpty()) {
        ...
        while ((pos = regExp.indexIn(text, pos)) != -1) {
            ...
            if (pos - prePos > 0) {
                QString tmpText = text.mid(prePos,
                                           pos - prePos); // 取匹配开始位置到匹配到的位置文本
...
//
void analysisAtMessage(const QString &content,
                       std::vector<StTextMessage> &messages, const std::map<QString, QString> &atUsers)
{
    auto tmpStr = QString("@%1 ").arg(DC.getSelfName().data());
    static QString allStr = "@all";

    // type groupAt
    if ((!atUsers.empty() &&
         atUsers.find(DC.getSelfXmppId().data()) != atUsers.end())
        || content.contains(tmpStr)) {
...
void analysisImageMessage(const QString &content, QString &imageLink,
                          qreal &width, qreal &height)
{
    ...
    if (regExp.indexIn(content) != -1) {
        ...
        if ("image" == type) {
            ...
            //
            if (width == 0 && height == 0) {
                QString localImg = st::GetImagePathByUrl(imageLink.toStdString()).data();
                QFileInfo info(localImg);
            ...
            st::Image::scaImageSize(width, height);
 ...
 * @param height
 */
void analysisEmoticonMessage(const QString &content, QString &pkgid,
                             QString &shortCut)
{
    QRegExp regExp("\\[obj type=[\\\\]?\"([^\"]*)[\\\\]?\" value=[\\\\]?\"([^\"]*)[\\\\]?\"(.*)\\]");
...
void analysisFileMessage(const QString &content, const QString &extendInfo,
                         StNetMessageResult &ret)
{
    ...
    if (doc.isEmpty()) {
        ...
        ret.file_info.fileIcon = st::getIconByFileName(ret.file_info.fileName);
    }
}

//
void analysisCommonTrdMessage(const QString &content, const QString &extendInfo,
                              StNetMessageResult &ret)
{
    QString data = extendInfo;
...
// code
void analysisCodeMessage(const QString &content, const QString &extendInfo,
                         StNetMessageResult &ret)
{
    QString data = extendInfo;
...
//
void analysisVideoMessage(const QString &content, const QString &extendInfo,
                          StNetMessageResult &ret)
{
    QString data = extendInfo;
    ...
    if (doc.isEmpty()) {
        ...
        //
        st::Image::scaImageSize(ret.video.width, ret.video.height);
    }
}
