...
void HeadPhotoLab::paintEvent(QPaintEvent *e)
{
    ...
    if (_mov) {
        ...
        if (index > 0) {
            if (index >= _pixmap.size()) {
                ...
                pixmap = st::qimage::scaledPixmap(pixmap, _radius * dpi * 2);
    ...
    if (pixmap.isNull()) {
        if (_pixmap.empty()) {
            pixmap = st::qimage::loadImage(_imagePath, true, true, _radius * 2 * dpi);
            _pixmap.push_back(pixmap);
        } else {
            pixmap = _pixmap[0];
        }
...
void HeadPhotoLab::initMovie()
{
    ...
    if (_startMovie && !_imagePath.isEmpty()) {
        QString suff = st::qimage::getRealImageSuffix(_imagePath);
        ...
        if (suff == "GIF") {
            ...
            if (count > 0) {
                ...
                _mov->start();
                _mov->setSpeed(80);
                connect(_mov, &QMovie::frameChanged, this, [this]() {
                    if (_mov) {
                        update();
                    }
                });
