...
FileSendReceiveMessItem::FileSendReceiveMessItem(const StNetMessageResult
                                                 &msgInfo, QWidget *parent) :
...
/**
  ...
  * @date 2018.10.22
  */
void FileSendReceiveMessItem::setProcess(double speed, double dtotal,
                                         double dnow, double utotal, double unow)
{
    // 发送文件切换会话处理
    if (st::entity::MessageDirectionSent == _msgInfo.direction && utotal > 0) {
...
void FileSendReceiveMessItem::initMenu()
{
    ...
    connect(_openFileAct, &QAction::triggered, this,
            &FileSendReceiveMessItem::onOpenFile);
    connect(this, &FileSendReceiveMessItem::sgOpenFile, this,
            &FileSendReceiveMessItem::onOpenFile, Qt::QueuedConnection);
}
...
void FileSendReceiveMessItem::setData()
{
    ...
    if (!_fileIcons.value(suffix).isEmpty()) {
        _contentTopFrmIconLab->setHead(_fileIcons.value(suffix),
                                       _contentTopFrmIconLabSize.width() / 2, false, false, true);
    }

    else {
        _contentTopFrmIconLab->setHead(FILEICON_UNKNOWN,
                                       _contentTopFrmIconLabSize.width() / 2, false, false, true);
    }
...
void FileSendReceiveMessItem::sendNDownLoadFile(const QString &strUri,
...
void FileSendReceiveMessItem::downLoadFile()
{
    ...
    _contentButtomFrmOPenFileBtn->hide();
    std::string localPath = g_pMainPanel->getFileMsgLocalPath(
                                _msgInfo.msg_id.toStdString());
...
void FileSendReceiveMessItem::onMenuBtnChicked()
{
    if (_downLoadMenu && _contentButtomFrmMenuBtn) {
        QPoint pos1 = QPoint(_contentButtomFrmMenuBtn->rect().center().x(),
                             _contentButtomFrmMenuBtn->rect().bottom());
...
void FileSendReceiveMessItem::onSaveAsAct()
{
    ...
    QString historyPath = QString::fromStdString(DC.getHistoryDir());
    ...
    if (!filePath.isEmpty() && fileInfo.exists()) {
        std::string fileMd5 = st::utils::getFileMd5(filePath.toLocal8Bit().data());

        if (_msgInfo.file_info.fileMd5.toStdString() == fileMd5) {
            DC.setHistoryDir(QFileInfo(
                                 fileName).absoluteDir().absolutePath().toStdString());
    ...
    // 下载文件
    g_pMainPanel->setFileMsgLocalPath(_msgInfo.msg_id.toStdString(),
                                      fileName.toStdString());
    downLoadFile();
}
...
void FileSendReceiveMessItem::onDownloadFailed()
{
    ...
    if (st::entity::MessageDirectionReceive == _msgInfo.direction) {
        ...
        _contentButtomFrmProgressBar->hide();
        _contentButtomFrmOPenFileBtn->hide();
    } else if (st::entity::MessageDirectionSent == _msgInfo.direction) {
        _contentButtomFrmProgressBar->hide();
        _contentButtomFrmOPenFileBtn->show();
        _contentButtomFrmMessLab->setText(tr("上传成功"));
    }
...
void FileSendReceiveMessItem::downloadOrUploadSuccess()
{
    //    if (compare_doule_Equal(100, process) && _contentButtomFrmMessLab) //
    {
        ...
        if (!filePath.isEmpty()) {
            g_pMainPanel->makeFileLink(filePath,
                                       _msgInfo.file_info.fileMd5.toStdString().data());
        }
