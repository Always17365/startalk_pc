...
#ifndef STALK_V2_MEDALDETAILWND_H
#define STALK_V2_MEDALDETAILWND_H
...
public:
    void showMedalDetail(const st::entity::ImMedalList &medalInfo, bool isSelf, int status);
    inline QWidget *getMoveWgt() { return _topFrm;};
    void setMedalUsers(int id, const std::vector<st::StMedalUser> &metalUsers);
    void onModifySuccess(int id, bool isWear);

Q_SIGNALS:
    void sgShowBack();
    void sgUpdateUsers();
    void sgShowUserList(const std::vector<st::StMedalUser> &);
...
private:
    QMutex _mutex;
    int _medalId = -1;
    std::vector<st::StMedalUser> _metalUsers;
...
#endif //STALK_V2_MEDALDETAILWND_H
