...
//                                        static std::string user, password;
                    ...
                    case SignalTypeRevoke:
                        {
                            ...
                            if (xmppMsg.ParseFromString(message->message()))
                            {
                                if (_pLogicBase && (xmppMsg.messagetype() == MessageTypeRevoke || xmppMsg.messagetype() == MessageTypeConsultRevoke))
                                    _pLogicBase->recvRevokeMessage(message->from(), xmppMsg.body(), xmppMsg.receivedtime());
                            }
                    ...
                    case SignalTypeWelcome:
                        {
                            ...
                            {
                                std::string plain;
                                plain = QTalk::Security::pwd_key_plain(_loginUser.username(), _password, _isNewLogin ? 1 : 2);
        ...
        */
        ...
        void ProtobufStack::sendAuthNoKey()
        {
            ...
            plantText << '\0'
                      ...
                      << '\0'
                      << _password;
            std::string key = base64_encode((unsigned char *)plantText.str().data(), plantText.str().size());
            //
            AuthMessage message;
            message.set_mechanism("PLAIN");
            message.set_authkey(key);
        ...
        void ProtobufStack::onTlsStarted()
        {
            if (_pLogicBase)
            {
                std::string plain = QTalk::Security::pwd_key_plain(_loginUser.username(), _password, _isNewLogin ? 1 : 2);
                sendAuthWithKey(plain);
            }
        }

        void ProtobufStack::sendIQMessage(IQMessage *message, QTalk::Entity::JID *toJid)
        {
            ProtoMessage package;
        ...
        /**
          ...
          * @date     2018/10/08
          */
        QTalk::Entity::JID *ProtobufStack::getLoginUser()
        {
            return &_loginUser;
        }
        ...
        void ProtobufStack::trancelateIQMsg(const IQMessage &message, IQMessageEvt *iqMsgEvt)
        {
        ...
        void ProtobufStack::trancelateHeader(const StringHeader &header, StStringHeader *stHeader)
        {
        ...
        void ProtobufStack::trancelateBody(const MessageBody &msgBody, StMessageBody *stBody)
        {
