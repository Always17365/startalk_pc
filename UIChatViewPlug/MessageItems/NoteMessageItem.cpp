...
NoteMessageItem::NoteMessageItem(const StNetMessageResult &msgInfo,
                                 QWidget *parent):
...
void NoteMessageItem::initContentLayout()
{
    ...
    if (!jsonDocument.isNull()) {
        ...
        connect(this, &NoteMessageItem::sgDownloadedIcon, this,
                &NoteMessageItem::setIcon, Qt::QueuedConnection);
        QString placeHolder = ":/chatview/image1/defaultShareIcon.png";
        QPixmap defaultPix = st::qimage::loadImage(placeHolder, false, true, 80, 40);
        ...
        if (imgUrl.isEmpty()) {
            ...
            if (QFile::exists(imgPath.data())) {
                iconLabel->setPixmap(st::qimage::loadImage(imgPath.data(), false, true, 80,
                                                              40));
            } else {
                iconLabel->setPixmap(defaultPix);
        ...
        _contentFrm->setFixedSize(10 + 10 + titleLabel->width() + iconLabel->width(),
                                  15 + 15 + titleLabel->height() + tagLabel->height() + iconLabel->height());
    }
}
...
{
    QPixmap pixmap = st::qimage::loadImage(iconPath, false, true, 80, 40);
