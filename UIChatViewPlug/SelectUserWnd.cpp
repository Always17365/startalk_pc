        ...
        const QModelIndex &source_parent) const
{
    ...
    if (!ret)
    {
        ...
        QString xmppId = index.data(EM_DATATYPE_XMPPID).toString().section("@", 0,
                         0).toLower();
        QString realJid = index.data(EM_DATATYPE_REALJID).toString().section("@", 0,
                          0).toLower();
        QString name = index.data(EM_DATATYPE_NAME).toString().toLower();
        QString searchKey = index.data(EM_DATATYPE_SEARCHKEY).toString().section("@", 0,
                            0).toLower();
        //
...
void
SelectUserWndDelegate::paint(QPainter *painter,
...
QSize SelectUserWndDelegate::sizeHint(const QStyleOptionViewItem &option,
...
bool SelectUserWndDelegate::editorEvent(QEvent *event,
...
void SelectUserWnd::initUi()
{
    ...
    bottomLay->addItem(new QSpacerItem(1, 1, QSizePolicy::Expanding,
                                       QSizePolicy::Fixed));
    ...
    connect(_pBtnGroup, SIGNAL(buttonClicked(int)), this,
            SLOT(onBtnGroupClicked(int)));
    connect(this, &SelectUserWnd::sgUpdateUi, this, &SelectUserWnd::updateUi);
    connect(selectUserWndDelegate, &SelectUserWndDelegate::itemCheckChanged, this,
