...
//
QPixmap qimage::loadImage(const QString &srcPath, bool save, bool scaled,
...
QPixmap qimage::loadCirclePixmap(const QString &srcPath, const int &radius,
                                 bool isGrey)
{
...
QPixmap qimage::generateGreyPixmap(const QPixmap &src)
{
    ...
    auto *newImage = new QImage(origin.width(), origin.height(),
...
QString qimage::getRealImagePath(const QString &filePath)
{
    ...
    if (imageInfo.suffix().toUpper() == suffix) {
        ...
        QString newPath = QString("%1/%2.%3").arg(imageInfo.absolutePath()).arg(
                              imageInfo.baseName()).arg(suffix);
...
QString qimage::getGifImagePath(const QString &srcPath, qreal &width,
                                qreal &height)
{
    if (srcPath.isEmpty()) {
        return srcPath;
    ...
    QString dstPath = QString("%1/%2_gif.png").arg(imageInfo.absolutePath(),
                                                   imageInfo.baseName());
...
QString qimage::getGifImagePathNoMark(const QString &srcPath)
{
    ...
    QFileInfo imageInfo(srcPath);
    QString dstPath = QString("%1/%2.png").arg(imageInfo.absolutePath(),
                                               imageInfo.baseName());
