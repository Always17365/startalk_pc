...
void MedalMind::initContentLayout()
{
    ...
    if (document.isNull()) {
        ...
        allStr.replace(highlightStr,
                       QString("<span style='color:rgba(0,202,190,1)'>%1</span>").arg(highlightStr));
        contentLabel->setText("<span>" + allStr + "</span>");
    }

    contentLabel->adjustSize();
    tipLabel->adjustSize();
    _contentFrm->setFixedHeight(contentLabel->height()  + tipLabel->height() + 20 +
                                10);
}
...
void MedalMind::mousePressEvent(QMouseEvent *event)
{
    if (event->button() == Qt::LeftButton
        ...
        QString userId = DC.getSelfXmppId().data();
