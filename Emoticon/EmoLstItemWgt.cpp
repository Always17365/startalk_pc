...
EmoLstItemWgt::~EmoLstItemWgt() = default;
...
void EmoLstItemWgt::paintEvent(QPaintEvent *e)
{
    ...
    if (_isChecked) {
        //
        pen.setColor(st::StyleDefine::instance().getNavNameFontColor());
        //painter.setBrush(QBrush(checkCor));
        //painter.drawRect(QRect(0, 0, 5, height()));
    } else {
        pen.setColor(st::StyleDefine::instance().getNavContentFontColor());
    }
