    ...
    QFrame(parent), _readSts(0)
{
    ...
    if (_msgInfo.state == 0
        ...
        _sending = new HeadPhotoLab(":/QTalk/image1/loading.gif", 10, false, false,
                                    true, this);
    ...
    connect(this, &MessageItemBase::sgDisconnected, this,
            &MessageItemBase::onDisconnected, Qt::QueuedConnection);
    connect(this, &MessageItemBase::sgUpdateUserMedal, this,
            &MessageItemBase::updateUserMedal, Qt::QueuedConnection);
}

MessageItemBase::~MessageItemBase()
{
#ifdef QT_DEBUG
    std::cout << "delete message item " << _msgInfo.msg_id.toStdString() << " -> "
              << _msgInfo.body.toStdString() << std::endl;
#endif
}
    ...
    if ((o == _headLab /*|| (_nameLab && o == _nameLab)*/) && g_pMainPanel) {
        ...
        if (e->type() == QEvent::MouseButtonPress) {
            if (g_pMainPanel && g_pMainPanel->getConnectStatus()) {
                //
                int ret = QtMessageBox::question(g_pMainPanel, tr("提醒"),
                                                 tr("确认重发此消息？"));
