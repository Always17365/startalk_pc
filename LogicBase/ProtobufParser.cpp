...
namespace Protocol {
    ...
    void tea_encrypt(uint32_t *v, uint32_t *k) {
        //printf("%.8x,%.8x\n",v[0],v[1]);
        uint32_t v0 = static_cast<uint32_t>(EndianIntConvertLToBig(
                v[0])), v1 = static_cast<uint32_t>(EndianIntConvertLToBig(v[1])),/*vn0=v[0],vn1=v[1],*/ sum = 0, i;
        //printf("%.8x,%.8x\n",v0,v1);
        /* set up */
        uint32_t delta = 0x9e3779b9;                     /* a key schedule constant */
        uint32_t k0 = EndianIntConvertBToLittle(k[0]), k1 = EndianIntConvertBToLittle(
                k[1]), k2 = EndianIntConvertBToLittle(k[2]), k3 = EndianIntConvertBToLittle(k[3]);   /* cache key */
        ...
        /* end cycle */
        //printf("s %.8x,%.8x\n",v0,v1);
        v[0] = static_cast<uint32_t>(EndianIntConvertLToBig(v0));
        v[1] = static_cast<uint32_t>(EndianIntConvertLToBig(v1));
        //printf("r %.8x,%.8x\n",v[0],v[1]);
    }

    void tea_decrypt(uint32_t *v, uint32_t *k) {
        uint32_t v0 = static_cast<uint32_t>(EndianIntConvertLToBig(
                v[0])), v1 = static_cast<uint32_t>(EndianIntConvertLToBig(
    ...
    uint32_t parse__uint32(std::string *string) {
        google::protobuf::io::CodedInputStream stream(
                reinterpret_cast<const google::protobuf::uint8 *>(string->c_str()), static_cast<int>(string->length()));
        uint32_t result;
    ...
    bool parseMessageWithHeader(ProtoHeader *header, ProtoMessage *message, uint32_t tea_key[4]) {
        ...
        if (header) {
            if (header->has_options()) {
                ...
                if (header->options() == 0x01) {
                    ...
                    std::string output = decodeWithGZip(&msg);

                    if (output.length() > 0) {
                        message->ParseFromString(output);
                        return message->IsInitialized();
                    } else {
                        throw QTalk::Excetpion::ParseFailedException("unGZip failed.");
                    }
    ...
    bool ProtobufParser::parseOneObject(ProtoMessage *message, const uint8_t *msg, size_t len) {
            ...
            do {
                ...
                if (remain >= 1) {
                    ...
                    for (i = 1; i <= count; i++) {
                        ...
                        /* code */ //整数最多10位
                        varintcount = scan__varint(i, reinterpret_cast<const uint8_t *>(buff.bytes() + pos));
    ...
    bool ProtobufParser::parseAllObjects(std::vector<ProtoMessage> objects, const uint8_t *msg, size_t len) {
        return false;
    }

    std::string ProtobufParser::buildPackageForProtoMessage(const ProtoMessage *message) {
        if (message) {
...
//            static uint32_t MAX_BUF_LENGTH = 0;
            ...
            if (len > 200) {
                ...
                std::string encodeBuff = encodeWithGZip(&buffer);
