...
CacheDataDao::CacheDataDao(st::sqlite::database *sqlDb)
...
bool CacheDataDao::insertUserId(const std::string &value)
{
    ...
    std::string sql =
        "INSERT OR REPLACE INTO IM_Cache_Data (`key`, `type`, `value`) values (?, ?, ?);";
    st::sqlite::statement query(*_pSqlDb, sql);
...
void CacheDataDao::getHotLines(std::string &hotLines)
{
    ...
    std::string sql =
        "SELECT `value` FROM IM_Cache_Data WHERE `key` = ? AND `type` = ?;";
    st::sqlite::statement query(*_pSqlDb, sql);
