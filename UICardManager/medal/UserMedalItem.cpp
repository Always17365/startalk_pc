...
void UserMedalItem::setUserMedal(const st::entity::ImMedalList &medalInfo,
                                 int status, bool isDetail)
{
    _isDetail = isDetail;
    ...
    if (_isDetail) {
        _imagePath = st::getMedalPath(medalInfo.bigLightIcon).data();
    } else {
        if ((status & 0x02) > 0) {
            _imagePath = st::getMedalPath(medalInfo.bigLightIcon).data();
            _text = tr("佩戴中");
            ...
            _imagePath = st::getMedalPath(medalInfo.bigLockIcon).data();
