...
void load(const StNetMessageResult &msgInfo)
{
    ...
    if (!jsonDocument.isNull()) {
        ...
        if (/**userDftBrowser ||**/
            ...
            //
            linkUrl = QString("%5username=%1&company=%2&group_id=%3&rk=%4").arg(
                          DC.getSelfUserId().data())
                      .arg("qunar").arg(msgInfo.xmpp_id).arg(DC.getServerAuthKey().data()).arg(
                          linkUrl);
...
CommonTrdInfoItem::CommonTrdInfoItem(const StNetMessageResult &msgInfo,
                                     QWidget *parent) :
...
void CommonTrdInfoItem::initContentLayout()
{
    ...
    if (jsonDocument.isNull()) {
        ...
        _contentFrm->setFixedSize(10 + 10 + _titleLab->width(),
                                  10 + 10 + _titleLab->height());
    } else {
        QJsonObject jsonObject = jsonDocument.object();
        bool allContent = jsonObject.value("showas667").toBool(false);
        allContent |= (_msgInfo.msg_type == st::entity::MessageTypeCommonTrdInfoV2);
        ...
        if (!_titleLab) {
            ...
            connect(_contentLab, &QTextEdit::cursorPositionChanged, this, [this]() {
        ...
        _contentLab->setText(desc);
        connect(this, &CommonTrdInfoItem::sgDownloadedIcon, this,
                &CommonTrdInfoItem::setIcon, Qt::QueuedConnection);
        QString imgUrl = jsonObject.value("img").toString();
        QString placeHolder = ":/chatview/image1/defaultShareIcon.png";
        QPixmap defaultPix = st::qimage::loadImage(placeHolder, false, true, 40, 40);

        if (imgUrl.isEmpty()) {
            _iconLab->setPixmap(defaultPix);
        } else {
            std::string imgPath = st::GetImagePathByUrl(imgUrl.toStdString());

            if (QFile::exists(imgPath.data())) {
                _iconLab->setPixmap(st::qimage::loadImage(imgPath.data(), false, true, 40,
                                                             40));
            } else {
                _iconLab->setPixmap(defaultPix);
                QPointer<CommonTrdInfoItem> pThis(this);
                QT_CONCURRENT_FUNC([pThis, imgUrl]() {
                    std::string downloadFile = ChatMsgManager::getLocalFilePath(
                                                   imgUrl.toStdString());
...
{
    QPixmap pixmap = st::qimage::loadImage(iconPath, false, true, 40, 40);
