...
void GroupChatSidebar::updateGroupMember(const std::string &memberJid,
...
void GroupChatSidebar::updateGroupMember(const GroupMemberMessage &e)
{
    ...
    const std::map<std::string, st::StUserCard> &members = e.members;
    ...
    for (; it != members.cend(); it++) {
        //
        bool isOnline = DC.isOnline(entity::JID(it->first).basename());

        if (isOnline) {
            onlineUserSize++;
        }

        auto itFind = std::find(deletedMembers.begin(), deletedMembers.end(),
                                it->first);
...
void GroupChatSidebar::initUi()
{
    ...
    connect(this, &GroupChatSidebar::deleteMember, _pGroupMember,
            &GroupMember::deleteMember);
    connect(this, &GroupChatSidebar::updateGroupTopic, _pGroupTopic,
            &GroupTopic::setTopic);
}
...
//
void GroupChatSidebar::onUpdateGroupInfo(const QString &groupId,
