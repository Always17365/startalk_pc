...
int processCallback(void *clientp, double dltotal, double dlnow, double ultotal,
                    double ulnow)
{
    if (dltotal == dlnow == ultotal == ulnow && dltotal == 0) {
        return 0;
    }
...
static CURL *innerInit(const char *url, long timeout,
...
void QtHttpRequest::setRequestMethod(RequestMethod method)
{
    ...
    if (curl != nullptr) {
        switch (method) {
        case st::RequestMethod::POST:
            curl_easy_setopt(curl, CURLOPT_POST, 1);
            break;

        case st::RequestMethod::GET: {
...
void QtHttpRequest::appendPostData(const char *body, int )
{
    CURL *curl = _httpCore;
...
void st::QtHttpRequest::addFromFile(const std::string &filePath)
{
    CURL *curl = _httpCore;
...
/**
  ...
  * @date     2018/10/16
  */
void st::QtHttpRequest::setProcessCallback(const std::string &key,
